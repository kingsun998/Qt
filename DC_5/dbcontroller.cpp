...
dbSinglePortTest::dbSinglePortTest(QVector<SinglePortStruct>& vec,QVector<QString>& sensorId,QString type)
{
    this->vector.swap(vec);
    this->sensorId.swap(sensorId);
    this->type=type;
}
...
void createSinglePortTest(libxl::Sheet *sheet){
    if(sheet==nullptr){return;}

    int colnums=0;
    sheet->writeStr(0,colnums++,L"测试时间");
    sheet->writeStr(0,colnums++,L"传感器ID");
    sheet->writeStr(0,colnums++,L"电压值");
    sheet->writeStr(0,colnums++,L"电阻值");
    sheet->writeStr(0,colnums++,L"测试状态");
    sheet->writeStr(0,colnums++,L"测试花费时间(0.1s)");
}
...
/// 串口信息保存
///
void dbSinglePortTest::save(){

     QString current_date_time=getCurrentDate();
     QString filename;
     if(type=="water"){
        filename=QCoreApplication::applicationDirPath()+"/savefiles/WaterTest/"+current_date_time+".xls";
     }else{
        filename=QCoreApplication::applicationDirPath()+"/savefiles/HighTempTest/"+current_date_time+".xls";
     }
     qDebug()<<"single chart start to write";
     ...
     for (long line=base,i=0;i<vector.size();i++) {
         int col=0;
         if(line>settings.xlsxMaxline){
             sheet=book->addSheet(QString::number(book->sheetCount()).toStdWString().c_str());
             createSingleChartXlsx(sheet);
...
//         sheet->writeStr(0,colnums++,L"电阻值");
//         sheet->writeStr(0,colnums++,L"测试状态");
//         sheet->writeStr(0,colnums++,L"测试花费时间");
         sheet->writeStr(line, col++,vector[i].testtime.toStdWString().c_str());
         sheet->writeStr(line, col++,sensorId[i].toStdWString().c_str());
         sheet->writeStr(line, col++,QString::fromStdString(vector[i].voltage).toStdWString().c_str());
         sheet->writeStr(line, col++,QString::fromStdString(vector[i].resistance).toStdWString().c_str());
         sheet->writeStr(line, col++,QString::fromStdString(vector[i].testState).toStdWString().c_str());
         sheet->writeStr(line, col++,QString::fromStdString(vector[i].time).toStdWString().c_str());
         line++;
     }
     book->save(filename.toStdWString().c_str());
     book->release();
