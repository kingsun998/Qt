...
void BootLoader::resizeEvent(QResizeEvent *pEvent)
{
...
void BootLoader::ScanDevice(void)
{
    int DevHandle[20];
...
QString BootLoader::GetErrorString(int ErrorCode)
{
    QString strTemp;
...
void BootLoader::on_openFirmwareFilePushButton_clicked()
{
    QString fileName;
...
bool BootLoader::DeviceConfig(void)
{
    int ret;
...
void BootLoader::on_updateFirmwarePushButton_clicked()
{
    QTime time;
    time.start();//系统时间记录为当前时间
    int ret;
    int DeviceHandle = ui->deviceHandleComboBox->currentText().toInt(NULL,16);//设备号
    bool ConfFlag;
...
//                qDebug()<<"ENTER BOOT MODE";
                ...
                ret = CAN_BOOT_EraseApp(DeviceHandle,
                                        ...
                                        10000);
                if(ret != CAN_SUCCESS){
                    ui->nodeListTableWidget->item(r,4)->setText(QString("执行固件擦除指令失败！\r")+GetErrorString(ret));
                    return;
                }else{
    ...
    writeDataProcess.setFixedSize(250,100);
#endif
        ...
        for(int r=0;r<ui->nodeListTableWidget->rowCount();r++){
            ...
            if(ckb->isChecked()){
                ...
                ret = CAN_BOOT_SetAddrOffset(DeviceHandle,
                                             ...
                                             50);
                if(ret != CAN_SUCCESS){
                    ui->nodeListTableWidget->item(r,4)->setText(QString("设置地址偏移失败！\r")+GetErrorString(ret));
                    return;
                }
            }
        }
...
//        qDebug()<<"read_data_num = "<<read_data_num;
        ...
        if(ui->allNodeCheckBox->isChecked()){
            ...
            if(ret != CAN_SUCCESS){
                for(int r=0;r<ui->nodeListTableWidget->rowCount();r++){
                    ui->nodeListTableWidget->item(r,4)->setText(QString("发送数据失败！\r")+GetErrorString(ret));
                }
...
//                    qDebug()<<"CRC16 = "<<QString().sprintf("%04X",CRC16);
        ...
        for(int r=0;r<ui->nodeListTableWidget->rowCount();r++){
            ...
            if(ckb->isChecked()){
                ...
                ret = CAN_BOOT_WriteAppData(DeviceHandle,
                                             ...
                                             1000);
                if(ret != CAN_SUCCESS){
                    ui->nodeListTableWidget->item(r,4)->setText(QString("APP数据写入失败！\r")+GetErrorString(ret));
                    writeDataProcess.close();
                    return;
                }
            }
    ...
    if(ui->allNodeCheckBox->isChecked()){
        ...
        if(ret != CAN_SUCCESS){
            for(int r=0;r<ui->nodeListTableWidget->rowCount();r++){
                ui->nodeListTableWidget->item(r,4)->setText(QString("发送执行固件指令失败！\r")+GetErrorString(ret));
            }
            return;
        }
    }else{
...
void BootLoader::on_scanNodeAction_triggered()
{
    int ret;
    bool GotNode=false;
    int DeviceHandle = ui->deviceHandleComboBox->currentText().toInt(NULL,16);
...
//    qDebug()<<"3";
    ...
    scanNodeProcess.setFixedSize(250,100);
#endif
...
void BootLoader::on_contactUsAction_triggered()
{
    QString AboutStr;
#ifndef LANG_EN
...
void BootLoader::on_aboutAction_triggered()
{
    QString AboutStr;
...
void BootLoader::on_exitAction_triggered()
{
    this->close();
}
...
void BootLoader::on_actionAddNode_triggered()
{
...
void BootLoader::on_comboBoxIDType_currentIndexChanged(int index)
{
    if(index){
...
void BootLoader::on_actionScanDevice_triggered()
{
    ScanDevice();
}

void BootLoader::on_actionDocument_triggered()
{
    QDesktopServices::openUrl(QUrl::fromLocalFile("Bootloader协议.pdf"));
}

void BootLoader::on_biaodingpushButton_clicked()
{
    QTime time;
    time.start();//系统时间记录为当前时间
    int ret;
...
//                qDebug()<<"ENTER BOOT MODE";
                ...
                QCoreApplication::processEvents(QEventLoop::AllEvents);//防止界面卡死，可处理别的事件
               //发送五次设置偏移地址指令，利用OLD_cmd区别固件更新和数据标定
                int j=0;
                uint16_t BufferSize_test=0;
                for(j=0;j<5;j++){
                    CAN_BOOT_SetAddrOffset(DeviceHandle,
                                                          ui->channelIndexComboBox->currentIndex(),
                                                          NodeAddr,
                                                          0,
                                                          &BufferSize_test,
                ...
                ret = CAN_BOOT_EraseApp(DeviceHandle,
                                        ...
                                        10000);
                if(ret != CAN_SUCCESS){
                    ui->nodeListTableWidget->item(r,4)->setText(QString("执行标定数据擦除指令失败！\r")+GetErrorString(ret));
                    return;
                }else{
                    ui->nodeListTableWidget->item(r,4)->setText("标定数据擦除成功！");
                }
            }else{
                ui->nodeListTableWidget->item(r,4)->setText("获取固件信息失败！");
                return;
    ...
    writeDataProcess.setWindowTitle("Upgrade firmware");
#else
    QProgressDialog writeDataProcess("正在标定数据...","取消",0,firmwareFile.size(),this);
    writeDataProcess.setWindowTitle("标定数据");
    writeDataProcess.setFixedSize(250,100);
#endif
    writeDataProcess.setModal(true);
    writeDataProcess.show();
...
//            qDebug()<<"GET CHIP INFO "<<NodeAddr;
            ...
            if(ret == CAN_SUCCESS){
                ...
                ui->nodeListTableWidget->item(r,3)->setText(str);
                //更新状态
                ui->nodeListTableWidget->item(r,4)->setText("数据标定成功！");
            }else{
                ui->nodeListTableWidget->item(r,4)->setText("数据标定失败！");
            }
        }
    }
//    qDebug()<<time.elapsed()/1000.0<<"s";
...
void BootLoader::on_pushButton_clicked()
{
    ScanDevice();
}
