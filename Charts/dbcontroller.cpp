...
dbSingleChart::dbSingleChart(QMap<QString,QMap<QString,QVector<QString>>> &tp,QVector<QString> &time)
{
    this->map.swap(tp);
    this->time.swap(time);
...
void createSingleChartXlsx(libxl::Sheet *sheet){
    ...
    for(int i=0;i<linenums;i++){
        sheet->writeStr(0,colnums++,QString("数值_"+settings.company_usedate_names[i]).toStdWString().c_str());
    }
}
