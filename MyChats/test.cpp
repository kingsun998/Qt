...
int test::response_http(struct bufferevent *bev, const char *method, char *path)
{
    if(stricmp("GET", method) == 0){
        //get method ...
        strdecode(path, path);
        char *pf = &path[1];
        ...
        qDebug()<<QString("***** http Request Resource Path =  %s, pf = %s").arg(path, pf);
...
//        }
        ...
        {
            send_header(bev, 200, "OK", get_file_type(pf), dir.Size);
...
*/
const char *test::get_file_type(const char *name)
{
    const char* dot;
...
int test::send_file_to_http(const char *filename, struct bufferevent *bev)
{
//    QFile f(filename);
    ...
    while(  (ret = fread(buf, sizeof(buf),1, fd) ) )
    {
...
int test::send_header(struct bufferevent *bev, int no, const char* desp, const char *type, long len)
{
    char buf[256]={0};
...
int test::send_error(struct bufferevent *bev)
{
...
int test::send_dir(struct bufferevent *bev,const char *dirname)
{
...
static void conn_readcb(struct bufferevent *bev, void *user_data)
{
    ...
    {
        test.response_http(bev, method, path);
    }
    printf("******************** end call %s.........\n", __FUNCTION__);
}
...
void test::signal_cb(evutil_socket_t sig, short events, void *user_data)
{
...
void test::listener_cb(struct evconnlistener *listener,
 ...
 */
void test::strdecode(char *to, char *from)
{
    for ( ; *from != '\0'; ++to, ++from)
    {
...
void test::strencode(char* to, size_t tosize, const char* from)
{
    int tolen;
